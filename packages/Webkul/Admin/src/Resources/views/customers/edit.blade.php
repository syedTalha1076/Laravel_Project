    ...
    <div class="flex flex-col gap-4">
        <div class="flex items-center justify-between rounded-lg border border-gray-200 bg-white px-4 py-2 text-sm dark:border-gray-800 dark:bg-gray-900 dark:text-gray-300">
            <div class="flex flex-col gap-2">
                ...
                <div class="text-xl font-bold dark:text-white">Edit Customer</div>
            </div>
            <div class="flex items-center gap-x-2.5">
                ...
                <button type="submit" form="customer-edit-form" class="primary-button">Update Customer</button>
            </div>
        </div>
        ...
        <form id="customer-edit-form" action="{{ route('admin.customers.update', $customer->id) }}" method="POST">
            @csrf @method('PUT')
                    ...
                    <div class="box-shadow rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
                        ...
                        <div class="mt-4 flex flex-col gap-1">
                            ...
                            <textarea name="address" rows="3"
                                class="rounded-md border border-gray-200 bg-white px-3 py-2 text-sm text-gray-800 focus:border-brandColor focus:outline-none dark:border-gray-700 dark:bg-gray-800 dark:text-white">{{ old('address', $customer->address) }}</textarea>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="flex flex-col gap-3">
                            <div class="flex flex-col gap-1">
                                ...
                                <select name="registration_type"
                                    class="rounded-md border border-gray-200 bg-white px-3 py-2 text-sm text-gray-800 focus:border-brandColor focus:outline-none dark:border-gray-700 dark:bg-gray-800 dark:text-white">
                                    <option value="converted" @selected(old('registration_type',$customer->registration_type)=='converted')>Converted Lead</option>
                                    <option value="direct" @selected(old('registration_type',$customer->registration_type)=='direct')>Direct</option>
                                </select>
                            </div>
                            <div class="flex flex-col gap-1">
                                ...
                                <select name="lead_id"
                                    ...
                                    @foreach($leads as $lead)
                                        <option value="{{ $lead->id }}" @selected(old('lead_id',$customer->lead_id)==$lead->id)>{{ $lead->title }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="flex flex-col gap-1">
                                <label class="text-xs font-semibold text-gray-600 dark:text-gray-300">Hire / Join Date</label>
                                <input type="date" name="hire_date" value="{{ old('hire_date', optional($customer->hire_date)->format('Y-m-d')) }}"
                                    class="rounded-md border border-gray-200 bg-white px-3 py-2 text-sm text-gray-800 focus:border-brandColor focus:outline-none dark:border-gray-700 dark:bg-gray-800 dark:text-white">
                            </div>
                        </div>
                    </div>
